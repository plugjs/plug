...
if (! ('.ts' in require.extensions)) {
  if (_debug) console.log(`[ts-loader cjs]: Installing TS loader from "${__filename}"`)

  require.extensions['.ts'] = (module, filename) => {
    if (_debug) console.log(`[ts-loader cjs]: Transpiling "${filename}"`)
    ...
    if (_debug) options.banner = {
      js: 'console.log(`[ts-loader cjs]: Loaded "${__filename}" (pid=${process.pid})`);'
    }
    ...
    /* Report out any warning or error abd fail if there are errors */
    ...
    try {
      ...
      if (_debug) {
        console.log('[ts-loader cjs] >', filename)
        console.log(code.replace(/^/gm, '[ts-loader cjs] | '))
      }
